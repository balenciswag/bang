...
)
{
	...
	if (GetAsyncKeyState(VK_INSERT) & 1)
	{
		...
		if (gui::open)
		{
			SetCapture(window); // Capture mouse when menu opens
			SetForegroundWindow(window); // Bring window to front
		}
		else
		{
			ReleaseCapture(); // Release mouse when menu closes
		}
	}

	// Prevent specific key handling when menu is open
	if (gui::open)
	{
		if (message == WM_KEYDOWN && (wideParam == 'T' || wideParam == VK_TAB))
		{
			// Prevent 'T' or Tab key from being processed by the game
